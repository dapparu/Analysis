...
void SaturationCorrection::SetTree(TTree &tree)
{
    inputtree_ = &tree;
}
...
{
    if(historec_->GetEntries()>=3 && (nsat>0)) //je ne recupere les resultats du fit que pour des profils avec plus de dix entrees et ou il y a eu saturation
    {    
...
{   
    ...
    if(nsat>=2) nsat=2; //inclusif pour le nombre de strips saturees 
